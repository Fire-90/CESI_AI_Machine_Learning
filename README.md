...
1. **Traitement des données** :
   ```python
   # Ouvrir Traitement.ipynb
   # Exécuter toutes les cellules
   # Génère 3 fichiers CSV dans data/
   ```

2. **Analyse exploratoire** :
   ```python
   # Dans Traitement.ipynb (dernière section)
   analyze_and_show()
...
3. **Entraînement des modèles** :
   ...
   # Ouvrir Modele.ipynb
   # Exécuter toutes les cellules
   # Compare les 9 modèles avec métriques et visualisations
   ```
...
Ce projet respecte les 7 principes de l'IA éthique définis par la Commission Européenne :

1. **Interaction et contrôle humains** : Les prédictions aident à la décision, ne la remplacent pas
2. **Robustesse technique** : Validation croisée et métriques multiples garantissent la fiabilité
3. **Protection de la vie privée** : Anonymisation des données, pas d'identifiants personnels
4. **Transparence** : Feature Importance pour expliquer les décisions du modèle
5. **Non-discrimination** : Surveillance des biais via les matrices de confusion
6. **Bien-être sociétal** : Objectif d'amélioration des conditions de travail
7. **Responsabilité** : Mécanismes d'audit via les métriques et visualisations
...
Le classement est basé sur le **Recall** (capacité à ne pas rater un départ) et le **F1-Score**.

Les résultats détaillés sont disponibles dans `Modele.ipynb` avec :
- Tableau comparatif des 9 modèles
- Matrices de confusion individuelles
- Analyse des variables les plus influentes

### Insights Clés

Les principales causes de départ identifiées incluent :
- Variables liées à la satisfaction
- Nombre d'heures de travail moyennes
- Ancienneté dans l'entreprise
- Type de poste (JobRole)
...
Les références académiques et techniques utilisées sont détaillées dans le fichier `Plan_action.txt`.

---

## 👥 Contributeurs

Projet CESI - Module Intelligence Artificielle - Année 3
